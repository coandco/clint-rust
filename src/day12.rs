...
impl Coord {
    ...
    fn move_direction(&self, direction: &Heading, amount: i32) -> Self {
        ...
        *self + Coord{y: direction_unit.y * amount, x: direction_unit.x * amount}
    }
...
pub fn part_one(data: &Vec<(Op, i32)>) -> i32 {
    ...
    for (op, value) in data {
        match op {
            ...
            Op::F => {
                current_loc = current_loc.move_direction(&current_heading, *value)
            }
        }
    }
...
pub fn part_two(data: &Vec<(Op, i32)>) -> i32 {
    ...
    for (op, value) in data {
        match op {
            ...
            Op::F => {
                ship_loc += Coord{y:waypoint_loc.y*value, x:waypoint_loc.x*value}
            }
        }
    }
